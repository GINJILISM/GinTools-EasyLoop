...
1. iOS 共有シート経由インポートの安定化（最優先）
   - 現状は `Runner` 側の受け取り導線を暫定実装済みだが、共有元アプリや起動状態によって取り込みが不安定
   - 今後は Share Extension + App Group 構成を含めて正式方式へ移行
...
- 共有シートからの動画インポートは暫定実装のため、実機で再現性に揺れがある
- 共有起点の要件は「共有ボタン → EasyLoop選択 → エディター画面で即編集開始」だが、現状は完全達成していないケースがある
...
- 共有シートから受け取る対象（動画/画像/URL）を定義
- App Group を使って Extension → 本体アプリへ受け渡すデータ構造を固定
...
1. Share Extension の受け渡し仕様と最小プロトタイプ完成（※ まずは「共有受け取り→アプリ遷移」だけを先に固定し、編集機能連結は次段で実装）
2. iOS書き出し方式のPoC（2案以上）と比較結果確定（※ いきなり本実装へ進まず、1機能ずつ段階導入して安定性を確認）
3. GIF品質プリセットUI（※ iOS書き出し方式確定後に着手し、依存仕様の手戻りを避ける）
4. 配布運用ドキュメント完成（証明書更新、バージョニング）（※ Extension追加後の署名手順差分も反映）
5. CIで `analyze/test/build` 自動化（※ iOS実機テストは手動ゲートを残しつつ徐々に自動化）
