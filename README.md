...
### 実装前
- サムネイルの画質が低すぎる問題
- サウンドOn or Off設定　(ループトグルの隣に作る。トグルでアイコン切り替え。書き出しにも影響)
...
- iOS 共有シート連携の基盤実装（Share Extension + App Group + URL Scheme）
   - Share Extension から App Group 共有領域へ動画を書き出し、本体を URL Scheme で開く構成
   - 本体起動後、共有された動画を直接編集画面へ引き渡すところまで復旧済み
- Windows MSIX 開発用インストールスクリプト

## 2026-03 iOS実機再起動問題の経緯と、最終的に安定した内容

2026-03 に、iPhone / iPad 実機で以下の症状が発生しました。

- Xcode から直接起動した直後は動く
- いったんタスクキルすると、ホーム画面からの再起動で青画面のまま止まることがある
- デバッグ接続あり/なしで挙動が変わる
- `FBSceneManager ... scene-creation-failed` が繰り返し出るケースがある

### 切り分けで見えたこと

- 実機の起動不安定は、共有シート単体の問題ではなく **iOS 起動直後の初期化量が多すぎること** と、
  **Flutter 3.41 系の scene / implicit engine 構成から外れた独自初期化** の影響が大きかった
- さらに、以下は別系統のノイズ要因だった
  - 端末の空き容量不足（`No space left on device`）
  - 未信頼の開発者証明書
  - Debug + LLDB 接続による起動遅延

### 最終的に効いた対応

1. **iOS ネイティブ起動経路を標準寄りに戻した**
   - `AppDelegate` を `FlutterImplicitEngineDelegate` ベースに整理
   - scene ライフサイクルを使う前提に戻し、`SceneDelegate` を最小構成へ簡素化
   - `Info.plist` では scene manifest を維持しつつ、重複起動要因になりやすい `UIMainStoryboardFile` を外した
2. **Flutter 側の重い初期化を起動直後から外した**
   - `MediaKit.ensureInitialized()` を `main()` から外し、必要画面側へ遅延
   - 起動直後の自動再生や iOS 初回サムネイル生成を抑制
   - 同一パスの二重オープンを防ぐガードを追加
3. **実機確認を Debug 依存にしない運用へ変更した**
   - VS Code では `profile no-debug` を追加
   - 「Xcode からは動くが、ホーム画面再起動で止まる」を Debug ノイズ抜きで再現確認できるようにした

### 現在の整理

- **本体の再起動問題は解消**
- **共有シート → 本体起動 → 編集画面直行** まで復旧
- ただし iOS 起動経路は壊れやすいため、今後も変更時は回帰確認を必須とする

### 共有シート復旧で最終的に必要だったもの

1. **scene ベースで URL を受けること**
   - `SceneDelegate` で `openURLContexts` / `willConnectTo` を処理する
   - Share Extension からの `easyloop://import-shared?...` をここで拾う
2. **App Group 上の共有ファイルを本体側で materialize すること**
   - Extension 側では重い処理をせず、共有領域へ保存して本体へ通知する
   - 本体側でテンポラリへコピーしてから Flutter へ渡す
3. **Flutter 準備前の共有イベントを捨てないこと**
   - iOS ネイティブ側で保留キューを持つ
   - Flutter 側は起動時に `notifyFlutterReady` / `consumePendingOpenFiles` で回収する
4. **同一パスの二重 reopen を防ぐこと**
   - 起動直後の二重遷移防止ガードを維持する

### 今後ここを触るときの注意

以下を安易に消すと、**「本体は開くが編集画面に遷移しない」状態へ戻りやすい**。

- [ios/Runner/SceneDelegate.swift](ios/Runner/SceneDelegate.swift) の URL 受信処理
- [ios/Runner/AppDelegate.swift](ios/Runner/AppDelegate.swift) の保留キューと Flutter 準備通知処理
- [lib/src/services/launch_file_service.dart](lib/src/services/launch_file_service.dart) の `notifyFlutterReady` / `consumePendingOpenFiles`
- [lib/src/state/app_controller.dart](lib/src/state/app_controller.dart) の二重 reopen 防止ガード

変更時は最低限、以下を毎回確認すること。

1. ホーム画面から通常起動して編集画面まで入れる
2. 共有シートから `イージーループ` を選ぶと本体が開く
3. 開いた直後にインポート画面ではなく編集画面へ入る
4. 既に本体起動中でも共有した動画へ置き換わる
5. `profile no-debug` で実機再現確認する

詳細な経緯・検証メモは `readme_iOSDev.md` を参照。
...
- UI上に表示される文言は **`lib/src/ui/app_strings.dart`** を唯一の管理元として扱います。
- 新しいテキストを追加する際は、画面側へ直書きせず `AppStrings` に定義して参照してください。
- 運用ルールの詳細は `docs/llm-agent-string-policy.md` を参照してください。

## iOS開発ドキュメント

- 詳細は `readme_iOSDev.md` を参照（Xcodeでの実機/シミュレータ手順、Codex連携、ビルド検証チェックリスト）。
- リブランディングとリリース準備メモは `docs/release-branding-ja.md` を参照。

## 既知の課題（2026-02時点）

- 実機ビルド時は空き容量不足で失敗しやすいため、`DerivedData` の定期クリーンを推奨
- Share Extension のバージョン番号が親アプリとずれているため、Xcode の warning が出る
